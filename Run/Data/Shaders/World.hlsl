...
VertexToPixelData VertexMain( VertexInputData vertex )
{
	...
	outputToPS.v_isWater = 0;
	
	float4 localPos = float4( vertex.a_position, 1.0 );
	float4 worldPos = mul( b_modelMatrix, localPos );
	float animatedWaterVertexZ = 0.0;
	if (vertex.a_color.b == 1.0)
	{
		outputToPS.v_isWater = 1;
		animatedWaterVertexZ = 0.5 * sin(b_time * 200.0 + (0.1 * worldPos.x + 0.1 * worldPos.y));
	}
	worldPos += float4(0.0, 0.0, animatedWaterVertexZ, 0.0);
